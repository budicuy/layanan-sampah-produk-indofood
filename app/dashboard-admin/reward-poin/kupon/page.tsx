...
export default function ClaimedCouponsPage() {
  ...
  const fetchCoupons = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getClaimedCouponsData();
      setCoupons(data as unknown as CouponDetails[]);
    } catch (error) {
      console.error(error);
  ...
  const filteredCoupons = coupons.filter(
    (c) =>
      ...
      c.nasabah?.user?.name.toLowerCase().includes(searchQuery.toLowerCase()),
  );
